...
const App = () => {
  ...
  // Calculate total price of all games
  const calculateTotalPrice = () => {
    return data.reduce((total, game) => {
      // Convert price to number, default to 0 if null/undefined/invalid
  ...
  const formatPrice = (price) => {
    return price.toFixed(2); // Show 2 decimal places
  };
  ...
  // Add these handler functions to your App.js component
  // (Place them with your other handler functions like handleAddGame)

  // Handle editing a game (simple version for now)
  const handleEditGame = (game) => {
    // For now, just log - you can enhance this later
    console.log('Edit game:', game);
    alert(`Edit functionality for "${game.title}" - coming soon!`);
    // TODO: Implement edit modal or inline editing later
  };
  ...
  const handleDeleteGame = async (gameId) => {
    ...
    try {
      // Send DELETE request to admin endpoint
      ...
      // Remove game from local state (optimistic update)
  ...
  // Add this robust formatter to your App.js (around line 650)
  const formatCellValue = (header, value) => {
    // Handle price column specifically
    if (header === 'price') {
      if (value === null || value === undefined || value === '') {
        return ''; // Show empty for no price
      }
      const price = parseFloat(value);
      if (isNaN(price)) {
        return ''; // Show empty for invalid price
      }
        ...
        data.length === 0 ? (
          ...
          <div className={`table-container ${isAdmin ? 'admin-mode' : 'public-mode'}`}>
            <table className="games-table">
              ...
              <thead>
                <tr>
                  {/* Actions column header - only show in admin mode */}
                  {isAdmin && (
              ...
              <tbody>
                {sortedData.map((game, index) => (
                  <tr key={game.id || index}>
                    {/* Actions column cell - only show in admin mode */}
                    {isAdmin && (
